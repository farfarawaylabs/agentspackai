...
describe("remote official packs", () => {
	test("initializes, checks, and updates without a local pack path", async () => {
		...
		const artifacts = new Map([
			["0.26.0", serializePackArtifact(versionOne, { kind: "official" })],
		...
		expect(initialized.stdout).toContain("agents-pack-core@0.26.0");
		...
		expect(checked.stdout).toContain("Candidate: agents-pack-core@0.27.0");
...
async function createNextCorePack(): Promise<string> {
	...
	await writeFile(
		manifestPath,
		manifest.replace('version = "0.26.0"', 'version = "0.27.0"'),
	);
	await writeFile(
