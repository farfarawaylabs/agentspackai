...
describe("remote official packs", () => {
	test("initializes, checks, and updates without a local pack path", async () => {
		...
		const artifacts = new Map([
			["0.29.0", serializePackArtifact(versionOne, { kind: "official" })],
			["0.30.0", serializePackArtifact(versionTwo, { kind: "official" })],
		]);
		let latest = "0.29.0";
		let baseUrl = "";
		...
		expect(initialized.stdout).toContain("agents-pack-core@0.29.0");
		...
		expect(checked.stdout).toContain("Candidate: agents-pack-core@0.30.0");
...
async function createNextCorePack(): Promise<string> {
	...
	await writeFile(
		manifestPath,
		manifest.replace('version = "0.29.0"', 'version = "0.30.0"'),
	);
	await writeFile(
