...
| Refresh Repository Documentation | `ap-refresh-repo-docs` | Audits and updates repository documentation so it agrees with the implemented code, configuration, architecture, commands, and product intent. | [Open skill](../content/packs/core/skills/engineering/documentation/ap-refresh-repo-docs/SKILL.md) |
| Start a Development Session | `ap-start-dev-session` | Orients the agent to the project, technical context, and current progress before beginning the requested work. | [Open skill](../content/packs/core/skills/engineering/workflows/session/ap-start-dev-session/SKILL.md) |
